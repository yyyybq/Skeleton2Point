...
def objective(weights):
    ...
    for i in tqdm(range(len(label))):
        ...
        r = r11 * weights[0] + r22 * weights[1] #+ r33 * weights[2] + r44 * weights[3] + r55 * weights[4] + r66 * weights[5] + r77 * weights[6]
        r = np.argmax(r)
...
if __name__ == "__main__":
    ...
    if 'UCLA' in arg.dataset:
        ...
        if 'xsub' in arg.dataset:
            ...
            npz_data = np.load('/data/LMY/ntu_interaction/ntu120/NTU120_CSetnpz')
            label = np.where(npz_data['y_test'] > 0)[1]
    elif 'ntu' in arg.dataset:
        if 'xsub' in arg.dataset:
            npz_data = np.load('/data/fzb_data/CTR-GCN/data/ntu/NTU60_CS.npz')
            label = np.where(npz_data['y_test'] > 0)[1]
        elif 'xview' in arg.dataset:
            npz_data = np.load('/data/fzb_data/CTR-GCN/data/ntu/NTU60_CV.npz')
            label = np.where(npz_data['y_test'] > 0)[1]
    ...
    #     with open(arg.joint_k2_dir, 'rb') as r5:
    #         r5 = list(pickle.load(r5).items())
    # if arg.joint_motion_k2_dir is not None:
    #     with open(arg.joint_motion_k2_dir, 'rb') as r6:
    #         r6 = list(pickle.load(r6).items())
    # if arg.pointcloud_dir is not None:
    #     with open(arg.pointcloud_dir, 'rb') as r7:
    #         r7 = list(pickle.load(r7).items())
            
    space = [(0.25, 0.75) for i in range(2)]
