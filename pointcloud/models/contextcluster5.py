...
try:
    ...
    from pytorch3d import furthest_point_sample as furthest_point_sample
except:
...
class LocalGrouper(nn.Module):
    ...
    def forward(self, xyz, points):
        B, N, C = xyz.shape
        S = self.groups
        xyz = xyz.contiguous()  # xyz [btach, points, xyztp]
...
class PreExtraction(nn.Module):
    def __init__(self, channels, out_channels,  blocks=1, groups=1, res_expansion=1, bias=True,
        ...
        in_channels = 5+2*channels if use_xyz else 2*channels#ybq0615 3->5
...
class Model(nn.Module):
    def __init__(self, points=3200, class_num=120, embed_dim=64, groups=1, res_expansion=1.0,
        ...
        self.points = points
        self.embedding = ConvBNReLU1D(5, embed_dim, bias=bias, activation=activation)
    ...
    def forward(self, x):
        ...
        for i in range(self.stages):
            # Give xyz[b, p, 5] and fea[b, p, d], return new_xyz[b, g, 5] and new_fea[b, g, k, d]
...
def pointMLP_CoC(num_classes=120, **kwargs) -> Model:
    return Model(points=5000, class_num=num_classes, embed_dim=64, groups=1, res_expansion=1,
