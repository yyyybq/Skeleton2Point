...
class Spatial_MixFormer(nn.Module):
    ...
    def forward(self, x0):
        ...
        # norm_learn_A = torch.rand((3,5,25,25)).to("cuda:0")
        ...
        for i in range(self.num_subset):  
            ...
            # print("A_final:", A_final.shape)
            # print("atten  :", atten.shape) 
            # print("norm_learn_A", norm_learn_A.shape)
            A_final[:,i,:,:,:] = atten * 0.5 + norm_learn_A[i]
            # A_final[:, i, :, :, :] = atten * 0.5 + norm_learn_A[i].view(1, 1, 1, 1)
            # A_final[:, i, :, :, :] = 1#atten * 0.5 #+ torch.unsqueeze(norm_learn_A[i], dim=0)
